  ...
  "PensionInputs201617Controller" when {
    ...
    "onPageLoad with GET request" should {
      ...
      "have keystore with DC flag = false and DB flag = true values, should have DB input field" in new ControllerWithMockKeystore {
        ...
        status(result) shouldBe 200
        val htmlPage = contentAsString(await(result))
        htmlPage should include (s"""<input type="text" name="${DB_PREFIX}2016" id="${DB_PREFIX}2016" min="0" class="input--no-spinner" value='' """)
      }
      ...
      "have keystore with values when we revisit the same page displays values" in new ControllerWithMockKeystore {
        ...
        status(result) shouldBe 200
        val htmlPage = contentAsString(await(result))
        htmlPage should include (s"""<input type="text" name="${DB_PREFIX}2016" id="${DB_PREFIX}2016" min="0" class="input--no-spinner" value='400' """)
        htmlPage should include (s"""<input type="text" name="${DC_PREFIX}2016" id="${DC_PREFIX}2016" min="0" class="input--no-spinner" value='600' """)
      }
    }
